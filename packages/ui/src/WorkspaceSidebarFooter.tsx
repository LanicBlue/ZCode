...
import { useIsRemoteWebSession } from "@/store/remoteWebSessionStore.js";
...
export const WorkspaceSidebarFooter = memo(function WorkspaceSidebarFooterComponent({
  ...
  // 远控 Web 挂载形态：账号/登录态数据源是 IOAuthService 的缓存 user_info——凭据邻接
  // （桥上硬排除，见 remoteBridge.ts），远端拿不到也不该拿。整个账号触发区（头像、
  // 登录态徽标、用量/升级/登录登出菜单）隐藏，只保留右侧设置齿轮；主题/语言等偏好
  // 仍可从设置页调整。
  const hideAccountArea = useIsRemoteWebSession();
